    ...
    def generate_frame_train(self, netG, real_A_all, fake_B_pyr, start_gpu, is_first_frame):        
        ...
        for t in range(n_frames_load):
            ...
            for s in range(n_scales):
                ...
                ####add begin
#                print('fake_B_pyr[si]', fake_B_pyr[si].size())
#                temp_size2 =  fake_B.cuda(dest_id)
#                print('fake_B.cuda(dest_id)', temp_size2.size())
#                temp_size = fake_B.unsqueeze(1).cuda(dest_id)
#                print('fake_B.unsqueeze(1).cuda(dest_id)', temp_size.size())
                ####add end
    ...
    def generate_first_frame(self, real_A, real_B, pool_map=None):
        ...
        if self.opt.no_first_img:          # model also generates first frame            
            ...
            for i in range(tG-1):                
                ...
                ##fake_B = self.netG_i.forward(real_A[:,i], feat_map).unsqueeze(1)  ##replaced with following
                ##add begin
    ...
    def load_single_G(self): # load the model that generates the first frame
        ...
        if 'City' in self.opt.dataroot:
            ...
            single_path = self.opt.checkpoints_dir + '/label2city_single/'
            if opt.loadSize == 512:
        ...
        ###add begin
        elif 'WX_seq_data_630' in self.opt.dataroot:
            single_path = '/gscratch/liulab/weisix/GAN_HE-CK8/checkpoints/frame_seq_630_t1/'
            load_path = single_path + 'latest_net_G.pth'
            netG = networks.define_G(3, 1, 0, 64, 'unet_512', 3, 'batch', 0, self.gpu_ids,opt)
