...
def define_G(input_nc, output_nc, prev_output_nc, ngf, which_model_netG, n_downsampling, norm, scale, gpu_ids=[], opt=[]):
    ...
    ##add begin
        ...
        netG = UnetGenerator(input_nc, output_nc, 9, ngf, norm_layer=norm_layer, use_dropout=True)
        netG = init_net(netG)
    ##add end
    ...
    ##add begin
    if not which_model_netG == 'unet_512':
    ##add end
        netG.apply(weights_init) #identation
...
#        print('img_raw',img_raw.size())
 ...
 #           print('img_prev',img_prev.size())
 #           temp3=img_prev[:,-3:,...].cuda(gpu_id)
 #           print('img_prev[:,-3:,...].cuda(gpu_id)',temp3.size())
 #           print('flow',flow.size())
            
            img_warp = self.resample(img_prev[:,-3:,...].cuda(gpu_id), flow).cuda(gpu_id) #changed from -3 to -1, changed back
 #           print('img_warp',img_warp.size())
...
#        print('img_final',img_final.size())
